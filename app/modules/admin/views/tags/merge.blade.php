                        ...
                        {{ Form::open(['method' => 'POST', 'route' => ['admin.tags.postMerge'], 'id' => 'merge-tags-form']) }}
                            <div class="col-md-4">
                                <div class="form-group">
                                    <div class="original-tags row">
                                        {{ Form::label('tags[1]', 'Тег', ['class' => 'col-sm-2 control-label']) }}
                                        {{ Form::text('tags[1]', null, ['class' => 'form-control autocomplete', 'placeholder' => '']) }}
                                        {{ $errors->first('tags[1]') }}
                                    </div>
                                </div>
                            </div>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $("#search-result").on('click', '.add-to-input', function() {
            var $link = $(this);
            var linkText = $link.find('.text').text();
            if(inputNumber == 1) {
                $("[id^='tags']").val(linkText);
            } else {
                var plusHtml = '<button type="button" class="btn btn-default btn-circle btn-outline"><i class="glyphicon glyphicon-plus"></i></button>';
                var inputHtml = '<input value="'+ linkText +'" class="form-control autocomplete ui-autocomplete-input" placeholder="" name="tags['+inputNumber+']" id="tags['+inputNumber+']" type="text">';
                var deleteInputHtml = '<a type="button" class="btn btn-danger btn-circle"><i class="glyphicon glyphicon-remove"></i></a>';
                $('.original-tags').append(plusHtml + inputHtml + deleteInputHtml);
            }
            inputNumber++;
            $link.remove();
        });
